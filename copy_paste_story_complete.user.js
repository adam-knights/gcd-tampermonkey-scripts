...
(async function() {
    ...
    $("h1").after('<button type="button" class="btn-blue-editing inline" id="copy" style="margin-right: 10px"><i class="far fa-copy" style="color: blue"></i> Copy Data</button><button type="button" class="btn-blue-editing inline" id="paste"><i class="fas fa-paste"  style="color: blue"></i> Paste Data</button>');
    ...
    $("#paste").click(async function (e) {
        ...
        if (gcd_form_noncreator) {
            for (let i = 0; i < gcd_form_noncreator.length; i++) {
                ...
                if (controlName === 'genre') {
                    $(`input[type="checkbox"][name="genre"][value="${controlValue}"]`).prop('checked', true).change();
                }
                else if (controlValue === 'on') {
                    $(`#id_${controlName}`).prop('checked', true).change();
                }
        ...
        if (gcd_form_creatorobject) {
            ...
            for (let i = 1; i < totalFormsRequired; i++) {
                $('.dynamic-form-add a.btn-blue-editing:contains("Add Another")').first().click();
            }
